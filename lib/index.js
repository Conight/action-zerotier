...
async function run () {
  ...
  try {
    ...
    await execShellCommand(`echo '${sshPubKey}' > ~/.ssh/authorized_keys`)
    await execShellCommand('chmod go-w ~/')
    await execShellCommand('chmod 700 ~/.ssh')
    await execShellCommand('chmod 600 ~/.ssh/authorized_keys')
